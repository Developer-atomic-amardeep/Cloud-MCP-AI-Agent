#!/usr/bin/env python3
"""
Script to run the billing-cost-management-mcp-server with AWS credentials loaded from .env
"""
...
def main():
    """Start the billing server with AWS credentials from .env"""
    
    # Try to load .env from multiple locations
    env_locations = [
        Path(__file__).parent / ".env",  # Root directory
        Path(__file__).parent / "llm_client_server" / ".env",  # Client directory
        Path(__file__).parent / "billing-cost-management-mcp-server" / ".env",  # Server directory
    ]
    ...
    print("\n🚀 Starting billing-cost-management-mcp-server...")
    print("   Server will be available at: http://localhost:8050/sse/")
    print("   Press Ctrl+C to stop the server\n")
    
    # Path to the server script
    server_script = Path(__file__).parent / "billing-cost-management-mcp-server" / "awslabs" / "billing_cost_management_mcp_server" / "server.py"
